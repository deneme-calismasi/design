...
for t in range(49):
    ...
    dict_value = dict[t].values()
    value_list = list(dict_value)
    print("value_list :", value_list)

    data2 = np.array(value_list).T.tolist()
    print("data2 :", data2)
...
for record in value_list:
    treev.insert("", index='end', iid=start_range, values=(int(record[0]), float(record[1]), int(record[2])))
    start_range += 1
