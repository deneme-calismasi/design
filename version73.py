...
class ModbusOop(object):
    ...
    def window_table(self):
        ...
        for record in res[-(start_regs // 2):]:
            ...
            start3 = 45
            n = 0
            for z in range(len(value[0][:26])):
                self.canvas.create_text(start3, 140, text=value[0][n:n + 1])
                start3 += 60
                n += 1

            start5 = 195
            for t in range(len(value[0][27:35])):
                self.canvas.create_text(395, start5, text=value[0][n:n + 1])
                start5 += 40
                n += 1

            start4 = 45
            for k in range(len(value[0][34:])):
                self.canvas.create_text(start4, 530, text=value[0][n:n + 1])
                start4 += 60
                n += 1
