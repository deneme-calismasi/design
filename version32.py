...
def start_calendar():
    ...
    if date_time:
        ...
        calendar_time = hour + ':' + minutes + ':' + seconds

        print(calendar_date + ', ' + calendar_time)

        global start_date
        start_date = calendar_date + ', ' + calendar_time
        print(start_date)
...
def end_calendar():
    ...
    if date_time:
        ...
        print(calendar_date + ', ' + calendar_time)
        global end_date
        end_date = calendar_date + ', ' + calendar_time
        print(end_date)
...
# myquery = {"Sensor No": "2"}
myquery = {"Time": {"$gte": start_date, "$lt": end_date}}
...
xs_doc = list(
    mycol.find(
        {"$and": [{"Sensor No": "2"}, {"Time": {"$gte": start_date, "$lt": end_date}}]},
        {'_id': 0}))
print(xs_doc)
